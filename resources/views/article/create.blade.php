<x-main-layout>
    <h2 class="text-3xl font-extrabold dark:text-gray-50">New Article</h2>
    <form method="POST" action="{{ route('article.store') }}">
        ...
        <div class="mb-6 space-y-4">
            <div>
                ...
                <input type="text" name="title" id="title"
                    ...
                    placeholder="Title for the article" required>
                @error('title')
            ...
            <div>
                ...
                <textarea id="text" name="text" placeholder="Write something..." rows="12"
                    class="w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-violet-500 focus:ring-violet-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-violet-500 dark:focus:ring-violet-500"></textarea>
                @error('text')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <button type="submit"
            class="w-full rounded-lg bg-violet-400 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-violet-800 focus:outline-none focus:ring-4 focus:ring-violet-300 dark:bg-violet-400 dark:hover:bg-violet-700 dark:focus:ring-violet-800 sm:w-auto">Submit</button>
    </form>
